...
- 장문 콘솔 출력 금지: 대용량 결과는 파일(JSON/MD)로 저장하고, 대화에는 핵심 지표와 결론만 요약한다.
- 백테스트/워크포워드/OOS 결과는 `artifacts` 또는 `quant_runtime*` 산출물로 남기고, 본문에는 경로 + 핵심 수치만 보고한다.
- 전략 평가는 반드시 `가설 → 구현 → 백테스트 → OOS/스트레스 검증 → 승격/보류` 순서를 따른다.
- 명시적 승인 없이는 실주문 실행 금지. 전략 발굴 단계는 paper/read-only 검증을 기본으로 한다.
