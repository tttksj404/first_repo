...
<career_tracking>
When a user request or completed work in this repository materially maps to the user's target data/AI job competencies, append a concise entry to `_meta/career_competency_log.md`.

Target categories:
- Data analysis, planning, design, cleansing, optimization
- Data pipeline and system integration development
- Broader data work support such as business insight generation or customer segmentation
- Capability growth in data extraction/processing/integration, logical data structuring, generative AI architecture understanding, and technical communication

Logging rules:
- Do not force every conversation into these categories. Skip logging when the connection is weak or incidental.
- Log only once per meaningful task or conversation turn cluster, after the useful outcome is clear.
- Keep each entry compact and practical: date, conversation/topic, what was done, mapped competencies, and the next skill to sharpen.
- Prefer concrete wording tied to the actual task, not generic career advice.
</career_tracking>
