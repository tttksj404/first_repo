...
- 결과 보고는 요약 우선: 핵심 지표(샤프, MDD, 수익률, 거래수)와 판정만 먼저 제시.
- 대형 로그/테이블 전체 붙여넣기 금지: 원본은 파일로 저장하고 경로만 공유.
- 탐색 루프는 단계별로 저장: `가설별 아티팩트`를 남겨 재실행 없이 비교 가능하게 유지.
- 전략 후보가 많을 때는 1차 거친 필터(거래수/비용 반영 수익률/드로다운)로 줄인 뒤 정밀 검증.
