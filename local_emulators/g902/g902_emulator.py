"""G902 paper-live emulator — G405 + Bitwise 2025 Trendwise gate (BTC EMA10>EMA20).
Backtest verdict: PnL +15% vs G405, Sharpe 2.48 -> 3.61 (verify_btc_timing/ensemble_results.json).
Capital halved (SIZE_PCT 0.2 -> 0.1) because runs in parallel with G902-session for E5 ensemble.
"""
...
EQUITY_USD=100.0; SIZE_PCT=0.1; LEVERAGE=20.0; THRESHOLD=80; HOLD_BARS=24
...
def main():
    log(f"=== G902 emulator START (S1+Trendwise) === eq=${EQUITY_USD} size={SIZE_PCT} lev={LEVERAGE}x thr={THRESHOLD} hold={HOLD_BARS}h max_conc={MAX_CONC} atr_guard={ATR_GUARD_PCT}% TW=EMA{BTC_EMA_FAST_DAYS}d/EMA{BTC_EMA_SLOW_DAYS}d")
