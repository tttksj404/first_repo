"""G904 paper-live emulator — G902 aggressive variant (lev 30x, size 0.30).
Same logic as G902 (S1+Trendwise) but scaled up for higher absolute PnL.
Backtest: 8 trades/yr, +$245, MDD -56.5%, Sharpe 3.16
(verify_btc_timing/lowfreq_highlev_results.json - thr80 lev30 size0.30 +TW).

Trade-off vs G902:
  + 3x absolute PnL (+$87 -> +$245)
  - 2.4x MDD risk (-23.8% -> -56.5%)
  - same Sharpe family (~3.2)
"""
...
EQUITY_USD=100.0; SIZE_PCT=0.30; LEVERAGE=30.0; THRESHOLD=80; HOLD_BARS=24
MAX_CONC=5; ATR_GUARD_PCT=6.0; COST_BPS_RT=16.0; CYCLE_SECONDS=300
BINANCE_KLINES="https://api.binance.com/api/v3/klines"
# Bitwise 2025 Trendwise gate: only enter when BTC trend bullish
BTC_EMA_FAST_DAYS=10
BTC_EMA_SLOW_DAYS=20
...
def btc_trendwise_bullish():
    """Bitwise 2025 Trendwise gate: True iff BTC EMA(10d) > BTC EMA(20d) on 1h bars."""
    df=fetch_klines("BTCUSDT", limit=BTC_EMA_SLOW_DAYS*24+10)
    if df is None or len(df) < BTC_EMA_SLOW_DAYS*24:
        log(f"  TW gate: insufficient BTC bars, defaulting to BLOCK entries")
        return False
    close=df["close_price"]
    ema_fast=close.ewm(span=BTC_EMA_FAST_DAYS*24, adjust=False).mean().iloc[-1]
    ema_slow=close.ewm(span=BTC_EMA_SLOW_DAYS*24, adjust=False).mean().iloc[-1]
    bullish=float(ema_fast) > float(ema_slow)
    log(f"  TW gate: EMA{BTC_EMA_FAST_DAYS}d={ema_fast:.0f} EMA{BTC_EMA_SLOW_DAYS}d={ema_slow:.0f} -> {'PASS' if bullish else 'BLOCK'}")
    return bullish
...
def cycle(state):
    ...
    # Trendwise gate: skip new entries when BTC short EMA below long EMA
    tw_ok = btc_trendwise_bullish()
    if not tw_ok:
        state["open_positions"]=open_pos
        return  # exits already processed above; just no new entries this cycle
...
def main():
    log(f"=== G904 emulator START (S1+TW aggressive) === eq=${EQUITY_USD} size={SIZE_PCT} lev={LEVERAGE}x thr={THRESHOLD} hold={HOLD_BARS}h max_conc={MAX_CONC} atr_guard={ATR_GUARD_PCT}% TW=EMA{BTC_EMA_FAST_DAYS}d/EMA{BTC_EMA_SLOW_DAYS}d")
