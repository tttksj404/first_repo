...
OUTPUT_BASE="${1:-quant_runtime_armG}"
...
# Arm G: futures-active profile (futures bias, score_min=56)
export STRATEGY_PROFILE=autoresearch-futures-active
...
exec sh "$SCRIPT_DIR/quant_python.sh" -m quant_binance.runtime \
  ...
  --insecure-ssl \
  >> "$OUTPUT_BASE/_armG.out.log" \
  2>> "$OUTPUT_BASE/_armG.err.log"
