...
OUTPUT_BASE="${1:-quant_runtime_armF}"
...
# Arm F: autoresearch leverage-strong (max 25x leverage focus)
export STRATEGY_PROFILE=autoresearch-leverage-strong
...
exec sh "$SCRIPT_DIR/quant_python.sh" -m quant_binance.runtime \
  ...
  --insecure-ssl \
  >> "$OUTPUT_BASE/_armF.out.log" \
  2>> "$OUTPUT_BASE/_armF.err.log"
