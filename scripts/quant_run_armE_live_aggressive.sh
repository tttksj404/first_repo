...
OUTPUT_BASE="${1:-quant_runtime_armE}"
...
# Arm E: live-aggressive profile (less aggressive than ultra)
export STRATEGY_PROFILE=live-aggressive
...
exec sh "$SCRIPT_DIR/quant_python.sh" -m quant_binance.runtime \
  ...
  --insecure-ssl \
  >> "$OUTPUT_BASE/_armE.out.log" \
  2>> "$OUTPUT_BASE/_armE.err.log"
