#!/usr/bin/env python3
"""Phase G: Time-of-day filter (UTC hour bucket).

크립토는 24/7이라 한국/유럽/미국 세션별 변동성 다름.
- Asia: 0-8 UTC
- Europe: 7-16 UTC
- US: 13-21 UTC

각 STRONG에 대해 entry hour bucket를 제한했을 때 PF/net 개선되는지 확인.
"""
...
def simulate_with_hours(ind, btc_regime, sig_fn, start, end, tp, sl, mom_min, allowed_hours, ts):
    """allowed_hours: set of UTC hours (0-23) when entries are allowed."""
    trades = []
    ...
    for i in range(max(start, 50), end):
        if not in_pos:
            ...
            # hour filter (ts in ms)
            hour = int((ts[i] // 3600000) % 24)
            if hour not in allowed_hours: continue
...
def quick_eval(ind, btc, fn, folds, tp, sl, mom, hours, ts_arr):
    all_pnls = []; wf = 0
    for s, e in folds:
        ts = simulate_with_hours(ind, btc, fn, s, e, tp, sl, mom, hours, ts_arr)
...
HOUR_BUCKETS = {
    "all_24h": set(range(24)),
    "asia_0_8": set(range(0, 8)),
    "europe_7_16": set(range(7, 16)),
    "us_13_21": set(range(13, 21)),
    "asia_eu_0_16": set(range(0, 16)),
    "eu_us_7_21": set(range(7, 21)),
    "off_us_0_13": set(list(range(0, 13)) + [22, 23]),
    "weekend_proxy_skip_us_open": set(range(24)) - set(range(13, 16)),  # avoid US open spike
}


def run():
    print("Phase G: time-of-day entry filter")
    cache = {}; ts_cache = {}
    for sym in UNIVERSE:
        df = load_1h(sym)
        ts_cache[sym] = df[:, 0].astype(np.int64)
    ...
    for sig_name, sym, mom, tp, sl in STRONG:
        sig_fn = SIGNALS[sig_name]; ind = cache[sym]; ts_arr = ts_cache[sym]
        bl = quick_eval(ind, btc_regime, sig_fn, folds, tp, sl, mom, HOUR_BUCKETS["all_24h"], ts_arr)
        if bl is None: continue
        for bname, bhours in HOUR_BUCKETS.items():
            if bname == "all_24h": continue
            r = quick_eval(ind, btc_regime, sig_fn, folds, tp, sl, mom, bhours, ts_arr)
    ...
    out = Path("quant_runtime/output/auto4h/phaseG_timeofday.json")
    with open(out, "w") as f:
        json.dump({"results": results}, f, indent=2, default=str)
    improvements = [r for r in results if r["improved"]]
    print(f"\n[saved] {out}")
    print(f"Phase G runtime: {time.time()-t0:.1f}s, {len(improvements)} improvements")
