#!/usr/bin/env python3
"""Donchian channel breakout + EMA regime filter (trend-following) with OOS.

Alpha: trend-following — opposite of mean-reversion.
- Long: close > N-bar high AND ema50 > ema200 (uptrend regime)
- Short: close < N-bar low AND ema50 < ema200 (downtrend regime)
- Exit: ATR-based TP/SL or hold timeout

Universe: 20 symbols × 1h.
Train/Test: 70/30.

Output: quant_runtime/donchian_trend_oos_summary.json
"""
...
OUT = ROOT / "quant_runtime" / "donchian_trend_oos_summary.json"
...
def donch_high(high: np.ndarray, period: int, idx: int) -> float:
    s = max(0, idx - period)
    return float(np.max(high[s:idx])) if idx > s else 0.0


def donch_low(low: np.ndarray, period: int, idx: int) -> float:
    s = max(0, idx - period)
    return float(np.min(low[s:idx])) if idx > s else 0.0


@dataclass
class TV:
    donch_period: int       # breakout lookback
    require_regime: bool    # require ema50 vs ema200 alignment
    tp_atr: float
    sl_atr: float
    hold: int


def variant_label(v: TV) -> str:
    return f"d{v.donch_period}|reg{int(v.require_regime)}|tp{v.tp_atr}|sl{v.sl_atr}|h{v.hold}"


def simulate(arr, ind, v: TV, idx_start, idx_end, extra_bps=0.0):
    ema50, ema200, atr = ind
    close = arr[:, 4]
    high = arr[:, 2]
    low = arr[:, 3]
    ...
    i = max(idx_start, max(200, v.donch_period + 1))
    while i < end:
        if i < cooldown:
            i += 1
            continue
        if atr[i] <= 0:
            i += 1
            continue
        dh = donch_high(high, v.donch_period, i)
        dl = donch_low(low, v.donch_period, i)
        long_break = close[i] > dh and (not v.require_regime or ema50[i] > ema200[i])
        short_break = close[i] < dl and (not v.require_regime or ema50[i] < ema200[i])
...
def main() -> None:
    ...
    qualified = [
        r for r in rows
        if r["train"]["pnl"] > 0 and r["train"]["pf"] is not None and r["train"]["pf"] >= 1.1
    ...
    print()
    print(f"Donchian trend: {len(rows)} variants, {len(qualified)} OOS-qualified")
