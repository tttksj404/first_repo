...
OUTPUT_BASE="${1:-quant_runtime_armC}"
...
# Arm C = majors-only universe (BTC, ETH, SOL)
export UNIVERSE_SYMBOLS=BTCUSDT,ETHUSDT,SOLUSDT
...
exec sh "$SCRIPT_DIR/quant_python.sh" -m quant_binance.runtime \
  ...
  --insecure-ssl \
  >> "$OUTPUT_BASE/_armC.out.log" \
  2>> "$OUTPUT_BASE/_armC.err.log"
