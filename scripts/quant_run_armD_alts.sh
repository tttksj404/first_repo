...
OUTPUT_BASE="${1:-quant_runtime_armD}"
...
# Arm D = alts-heavy universe (smaller-cap focus)
export UNIVERSE_SYMBOLS=PEPEUSDT,DOGEUSDT,XRPUSDT,SOLUSDT
...
exec sh "$SCRIPT_DIR/quant_python.sh" -m quant_binance.runtime \
  ...
  --insecure-ssl \
  >> "$OUTPUT_BASE/_armD.out.log" \
  2>> "$OUTPUT_BASE/_armD.err.log"
