# Gemini Final Review Artifact

## Original user task
Final-review the Binance regime-switching quant design with the strongest available Gemini model before implementation.

## Final prompt sent to Gemini CLI
```text
Final review of this updated Binance quant regime-switching design. Judge only whether implementation can begin without inventing core behavior.
Return sections exactly as:
1. Verdict: APPROVE or REVISE
2. Remaining blockers
3. Any rule conflicts still present
4. Minimal next build order
...
2. Remaining blockers
- The timeframe for the "20-bar breakout level" used in `breakout_reference_price` is not explicitly stated (though 1h or 4h is implied by the trend context).
- The storage/retrieval mechanism for `median_forward_return_bps` (whether rolling in-memory or a precomputed daily lookup table) is left to implementation, though its mathematical definition is fully specified.

3. Any rule conflicts still present
- None. The evaluation precedence (Futures -> Spot -> Cash), missing data fallback (`DATA_INCOMPLETE` -> Cash), and explicitly defined tie-breaking logic for percentile ranks remove operational ambiguity.

4. Minimal next build order
- Phase 1a: Initialize project structure, config loader, and immutable `Snapshot` data classes.
- Phase 1b: Implement market data streams (Spot/Futures) and the snapshot builder.
- Phase 1c: Build the feature pipeline (primitive calculations, percentile rank / clamp normalizers).
- Phase 1d: Implement the regime scorer (Predictability Score, Edge Gate, and Regime Switching Rules).
- Phase 1e: Implement risk sizing, the mock/paper execution broker, and decision logger.
- Phase 1f: Implement the deterministic unit tests and the historical replay harness using the defined fixtures.
```

## Concise summary
Selected model: gemini-3.1-pro-preview
Final review after adding primitive feature definitions, explicit cost treatment, parity checks, and replay oracle requirements.

## Action items / next steps
- Compare Gemini verdict with internal critic/verifier verdicts.
- If no blockers remain, begin implementation scaffolding.
