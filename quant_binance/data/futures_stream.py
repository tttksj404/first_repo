...
def build_futures_streams(symbol: str, intervals: tuple[str, ...]) -> list[str]:
    base = symbol.lower()
    streams = [f"{base}@trade", f"{base}@bookTicker", f"{base}@markPrice"]
...
def parse_futures_trade_event(payload: dict[str, Any]) -> SpotTrade:
    return SpotTrade(
        symbol=payload["s"],
...
def parse_futures_book_ticker_event(payload: dict[str, Any]) -> TopOfBook:
...
def parse_futures_kline_event(payload: dict[str, Any]) -> KlineBar:
    kline = payload["k"]
    return KlineBar(
        symbol=payload["s"],
        interval=kline["i"],
...
def parse_mark_price_event(payload: dict[str, Any]) -> dict[str, float | datetime]:
    return {
        "symbol": payload["s"],
...
def parse_open_interest_event(payload: dict[str, Any]) -> dict[str, float | datetime]:
    event_time_ms = payload.get("time", payload.get("E"))
    return {
