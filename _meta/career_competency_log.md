...
### 2026-05-07 - Chunked high-WR alpha expansion and sleeve combination
- Summary: Continued the expanded-data alpha search by converting timeout-prone broad searches into chunk/resume jobs and validating whether new funding sleeves combine with the G090 hour-filter sleeve.
- What was done: Added chunk controls to the 5m lead-lag, funding-extreme, and G090-variant searches; ran the first funding/5m/G090 chunks; found multiple funding strict-pass candidates; and added a combination validator showing G090+G805740+G827758 reaches 32.05 trades/month with 71.44% WR, all years positive, and zero tested liquidations under conservative overlap constraints.
- Competency mapping: Quant experiment orchestration, resource-aware data pipeline execution, strategy ensemble validation, metric correction, evidence-backed technical communication.
- Skill sharpened next: Continue resumable chunk execution and add overlap-aware promotion gates so high-cadence candidates are selected on portfolio-taken trades rather than raw signal count.

### 2026-05-07 - G7835 Oracle paper deployment
- Summary: Converted the validated G7835 high-WR sleeve combination into an Oracle paper runtime and recovered access when the VM SSH/agent path was stuck.
- What was done: Built `deploy_g7835_to_oracle.py`, implemented the combined G090/funding paper emulator, checked active services for zero-entry candidates, preserved active G4692 because it had entries, recovered Oracle access via OCI soft reset, deployed `g7835-emulator.service`, fixed a Bitget `history-candles` limit mismatch, and verified heartbeats with zero fetch errors.
- Competency mapping: Strategy-to-runtime integration, cloud operations recovery, paper-trading deployment validation, data-source API drift debugging, technical communication.
- Skill sharpened next: Add an automated Oracle health probe that checks SSH, Oracle Cloud Agent, service heartbeats, and fetch-error counts before every deployment.

### 2026-05-06 - Oracle third paper-strategy candidate selection
- Summary: Compared local Bitget/Oracle strategy evidence to choose a third paper-only candidate that is structurally different from deployed breakout-style strategies.
- What was done: Reviewed G130x replacement searches, multi-family non-overlap results, realtime paper summaries, Bitget current funding/24h move snapshots, and narrowed the next candidate toward funding-fade or rotation evidence instead of another breakout variant.
- Competency mapping: Data analysis and optimization, experiment comparison, quant feature selection, paper/live safety separation, evidence-backed technical communication
- Skill sharpened next: Build a focused paper emulator for the chosen non-breakout family and require fresh paper outcomes before any live-facing promotion.
...
## 2026-05-04 - Entry-frequency strategy refresh (G151)
- Conversation/topic: Addressed low-entry/low-profit concern by proposing a higher-frequency strategy path grounded in existing in-repo strategy evidence.
- What was done: Reviewed `STRATEGIES.md`, `REGISTRY.md`, and batch/grid backtest artifacts, identified that the active PEPE-only override is structurally sparse for entries, created `G151_freq_boost_score75` as a single-variable child of `G150` (`futures_score_min: 76 -> 75`), generated candidate override artifact for runtime handoff, and documented validation gates for next replay/paper cycle.
- Mapped competencies: Quant strategy iteration, parameter-sweep interpretation, backtest evidence synthesis, controlled config experimentation, technical communication.
- Next skill to sharpen: Build a fast deterministic replay harness for same-session metric verification to avoid long `analyze_backtest` timeout bottlenecks.

## 2026-05-06 - Strategy discovery synthesis from failed quant searches
- Conversation/topic: Reframed the coin-program search from more parameter tuning toward new alpha-source discovery after recent strategy results underperformed.
- What was done: Aggregated registry and batch/grid artifacts across G18x/G19x/G21x/G300/G400/G500/G600/G700/G710/G800, isolated repeated failure modes around forced frequency, short-only variants, short holds, and extreme leverage, then documented a strategy-discovery roadmap with CH1 path validation plus liquidation/OI, leaderboard-fill, and 5m structure-breakout research lines.
- Mapped competencies: Quant evidence synthesis, experiment design, alpha-source decomposition, risk-aware validation planning, technical communication.
- Next skill to sharpen: Implement path-based validation for G900 so close-to-close backtest winners are tested against intrabar liquidation and drawdown risk.

## 2026-05-06 - Ensemble discovery and path-safe CH1 candidate
- Conversation/topic: Tested whether a novel ensemble of CH1, structure long/short, and funding-fade signals could outperform prior coin strategies for the user's small-capital context.
- What was done: Built `g900_ensemble_discovery.py`, ran 85 ensemble/backtest candidates across three periods with path-based TP/SL and liquidation checks, rejected cost-negative structure/funding sleeves, and promoted `G914_path_safe_ch1` as the best research candidate with zero tested liquidations.
- Mapped competencies: Strategy ensemble design, path-dependent backtesting, risk-control optimization, evidence-based candidate selection, technical communication.
- Next skill to sharpen: Extend the harness with liquidation/OI primary signals and funding-veto variants instead of naive parallel funding trades.

## 2026-05-06 - G915 exit/veto optimization
- Conversation/topic: Continued the path-safe strategy experiment after G914 by searching optimized TP/SL, hold, ATR, leverage-size, and funding-veto variants.
- What was done: Added `g915_exit_veto_search.py`, evaluated 864 candidates across three periods, found 192 passers, and promoted `G1165_exit_veto_best` with `10x`, `25%` size, `36h` hold, `TP 14%`, `SL 7.5%`, `ATR 3-8%`, and zero tested liquidations.
- Mapped competencies: Hyperparameter search design, path-dependent risk validation, quant candidate promotion, experiment-result documentation.
- Next skill to sharpen: Build a liquidation/OI primary-alpha harness and compare it against G1165 as an independent sleeve rather than a naive parallel filter.

## 2026-05-06 - Oracle paper deployment preparation for G1165
- Conversation/topic: Prepared the selected `G1165_exit_veto_best` strategy for Oracle paper execution while preserving capacity by removing the weakest existing paper strategy first.
- What was done: Added a dedicated deploy script that probes existing Oracle strategy state, chooses the lowest-PnL paper strategy excluding G1165, disables it, uploads the G1165 emulator/dependency, and starts a user-level systemd paper service once SSH is reachable.
- Mapped competencies: Strategy-to-runtime integration, deployment automation, operational risk control, evidence-backed blocker diagnosis, technical communication.
- Next skill to sharpen: Harden Oracle access recovery with valid OCI CLI config or an alternate bastion path so deployment and paper monitoring are not blocked by a single SSH endpoint.

## 2026-05-06 - Oracle g185-restored recovery and G1165 paper deployment
- Conversation/topic: Recovered the Oracle deployment path after the old g185 instance OCID/IP became stale and moved the selected G1165 strategy into paper execution.
- What was done: Identified the active `g185-restored` instance via OCI Search, updated local recovery config and SSH alias to `138.2.127.203`, deployed `G1165` as `g1165-emulator.service`, verified runtime `state.json`, and confirmed previous restored-VM strategy services were inactive with no comparable negative-PnL active strategy to disable.
- Mapped competencies: Cloud deployment troubleshooting, infrastructure identity/IP recovery, paper-trading runtime validation, operational evidence reporting, technical communication.
- Next skill to sharpen: Add persistent OCI metadata discovery to recovery tooling so stale instance IDs/IPs are auto-detected before manual Cloud Shell investigation is needed.

## 2026-05-06 - Portable Oracle access automation
- Conversation/topic: Reduced recurring Oracle SSH/IP drift burden so the user can manage paper trading from current PC, future PC, Mac, or Cloud Shell without hand-editing server details.
- What was done: Added `ensure_access.py` for OCI-based instance discovery, SSH config regeneration, key enrollment support, and connectivity verification; rewired Mac setup/recovery runners around it; hardened the VM SSH daemon on ports 22/443; enrolled existing PC/Mac public keys; and scheduled a daily health-check automation.
- Mapped competencies: Infrastructure automation, cross-environment access design, cloud recovery workflow, operational reliability, technical communication.
- Next skill to sharpen: Add a small status dashboard or notification sink so paper-service health and stale-IP recovery results are visible without opening the thread.

## 2026-05-06 - G1300 two-slot paper strategy search
- Conversation/topic: Searched for two additional Oracle paper-slot candidates beyond the deployed G1165 strategy using the user's ensemble/grid-search selection logic.
- What was done: Added `g1300_two_slot_search.py`, evaluated 1,152 CH1 slot variants across no-dead/top10 universes with path TP/SL and liquidation checks, penalized entry overlap with G1165, and promoted `G1995_top10_slot_ch1` plus `G1503_no_dead_expansion_slot` as the two bt-only paper candidates.
- Mapped competencies: Experiment design, hyperparameter search, overlap-aware portfolio selection, backtest evidence synthesis, risk-control validation.
- Next skill to sharpen: Build an integrated multi-service paper allocator so overlapping CH1-family services share capital limits instead of independently consuming margin.

## 2026-05-06 - G1995/G1503 paper-readiness verification
- Conversation/topic: Rechecked whether the two G1300 candidates fit the user's current Oracle paper setup before promotion from backtest artifacts to runtime services.
- What was done: Verified result JSON, cards, overrides, validation summaries, Python compile, diff hygiene, active G1165 Oracle state, and found a deployment-template mismatch where the generic Oracle deploy path lacks the TP/SL logic used by the backtests.
- Mapped competencies: Evidence-first validation, strategy-to-runtime drift detection, quant risk review, operational readiness assessment, technical communication.
- Next skill to sharpen: Generalize the Oracle paper emulator generator so every candidate is deployed from the same override schema used in backtest validation.

## 2026-05-06 - G1503 replacement search
- Conversation/topic: Reassessed the second paper-slot candidate after finding G1503 overlapped too heavily with the active G1165 and proposed G1995 slots.
- What was done: Computed direct G1503-vs-G1995 overlap, searched tail/non-top10/meme replacement surfaces, and found no strict PASS replacement; identified the non-top10 G3408 family as the best independent watchlist shape despite failing trade-count and annual-PnL gates.
- Mapped competencies: Portfolio overlap analysis, rejection discipline, search-space narrowing, quantitative candidate triage, technical communication.
- Next skill to sharpen: Move the second slot away from CH1-family signals and test liquidation/OI or leaderboard-flow alpha sources for genuinely independent entries.

## 2026-05-06 - Non-overlapping non-CH1 alpha exploration
- Conversation/topic: Explored strategy families that do not reuse CH1/Mingogogo entries so the second paper slot can be independent from G1165 and G1995.
- What was done: Re-ran liquidation-cascade reversal and inversion checks, rejected that proxy as cost-negative, then built `g1303_non_ch1_overlap_search.py` for breakout/overreaction/pullback price-action engines with explicit G1165/G1995 overlap scoring; found a long-only volume-breakout watchlist candidate with zero overlap but below strict promotion thresholds.
- Mapped competencies: Alternative alpha-source design, overlap-aware validation, hypothesis rejection, side-specific signal decomposition, quantitative evidence reporting.
- Next skill to sharpen: Turn the long-only breakout watchlist into a stricter paper probe only after improving WR/DD or adding a regime filter that preserves zero overlap.

## 2026-05-06 - Multi-family non-overlap strategy sweep
- Conversation/topic: Broadened the second-slot search across multiple non-CH1 alpha families instead of continuing parameter tweaks around one candidate.
- What was done: Added `g1304_multi_family_non_overlap_search.py` to compare breakout long/short, climax reversal, trend pullback, and funding-fade families against the same three-period path simulator plus G1165/G1995 overlap gates; confirmed no strict-pass candidate, kept two breakout-long watchlist variants, and diagnosed the best candidate's symbol/hour concentration.
- Mapped competencies: Multi-hypothesis experiment design, family-level performance attribution, overlap-aware portfolio construction, rejection discipline, technical communication.
- Next skill to sharpen: Validate breakout-long symbol/hour filters out-of-sample or via walk-forward selection before treating the filtered improvement as deployable.

## 2026-05-06 - G4006 breakout walk-forward filter validation
- Conversation/topic: Tested whether the non-CH1 breakout-long watchlist candidate's symbol/hour improvements survive out-of-sample selection.
- What was done: Added `g1305_breakout_walkforward_filter.py`, trained symbol/hour filters on earlier periods and tested them on later periods, confirmed the raw breakout baseline and top-8-symbol filter both pass watchlist gates across OOS24 and IS25 while keeping zero overlap with CH1 anchors.
- Mapped competencies: Walk-forward validation, anti-overfit filter testing, risk/return tradeoff analysis, paper-probe candidate selection, technical communication.
- Next skill to sharpen: Package the raw and top-symbol breakout variants as separate paper-probe cards only after aligning Oracle emulator support for non-CH1 breakout signals.

## 2026-05-06 - Non-CH1 breakout paper-probe packaging
- Conversation/topic: Implemented the G1503 replacement path as independent breakout paper-probe artifacts rather than forcing it into the CH1 Oracle runtime.
- What was done: Added G4006/G4007 cards, overrides, walk-forward validation summaries, and a dedicated Oracle deploy script that computes 24h breakout, momentum, volume-ratio, ATR, TP/SL, and time exits in the runtime emulator.
- Mapped competencies: Strategy-to-runtime integration, quantitative validation packaging, deployment drift control, operational risk communication.
- Next skill to sharpen: Run Oracle paper deployment and compare emulator fills against the backtest signal timestamps before promoting either breakout variant beyond paper-probe status.

## 2026-05-06 - Loose breakout adaptive-response experiment
- Conversation/topic: Tested whether the non-CH1 breakout signal can support more frequent entries when exits and sizing adapt to signal strength.
- What was done: Added G1306/G1307 experiment scripts, compared loose starter/standard/strong buckets plus controlled one-gate relaxations, and found weak breakout buckets destroy expectancy while strict/strong breakout remains the only pass.
- Mapped competencies: Hypothesis-driven strategy research, signal decomposition, adaptive exit testing, evidence-based rejection, risk-return attribution.
- Next skill to sharpen: Convert weak breakout states into watch/confirmation states rather than trade entries, then test delayed confirmation or failure-reclaim logic.

## 2026-05-06 - Weak breakout fast-stop rejection test
- Conversation/topic: Checked whether weak breakout entries can be salvaged with very fast stop-loss and failure-close exits.
- What was done: Added G1308 weak-breakout-only experiments that exclude strict G4006 entries and test 20 short-hold TP/SL/failure-exit variants; all failed because stop-loss frequency overwhelmed small take-profit wins.
- Mapped competencies: Downside-control experiment design, exit attribution, negative-result interpretation, strategy rejection discipline.
- Next skill to sharpen: Use weak breakouts as pre-entry alerts for delayed confirmation instead of immediate trades.

## 2026-05-06 - G4692 watch-confirm breakout strategy
- Conversation/topic: Built the best current non-CH1 breakout candidate by turning weak breakouts into watch states that require next-bar confirmation before entry.
- What was done: Added G1309 watch-confirm search, tested 121 delayed-confirmation variants, selected G4692 as the top score candidate, and packaged its card, overrides, and validation artifact with Bitget futures runtime requirements.
- Mapped competencies: State-machine signal design, delayed confirmation testing, model selection under overfit risk, quantitative strategy packaging, technical communication.
- Next skill to sharpen: Implement the persisted watch-state Oracle paper emulator and compare live paper fills against G1309 backtest event timestamps.

## 2026-05-06 - G4692 Oracle paper deployment
- Conversation/topic: Promoted the selected watch-confirm breakout candidate from local validation artifacts into the Oracle Bitget paper emulator.
- What was done: Updated the deployment script for the persisted watch-confirm state machine, fixed runtime config serialization and Bitget candle limit drift, deployed G4692 to the Oracle user service, and verified service health plus zero fetch errors.
- Mapped competencies: Data pipeline/system integration, runtime drift debugging, paper-trading operations, verification evidence collection, deployment communication.
- Next skill to sharpen: Monitor paper fills versus G1309 backtest signal timestamps and add automated alerts for stale heartbeat or fetch-error recurrence.

## 2026-05-06 - Third Oracle non-overlap strategy search
- Conversation/topic: Reframed the third Oracle slot around performance-first non-overlap rather than filling strategy count.
- What was done: Rejected weak liquidation/mirror/funding alternatives, searched non-breakout reversal/pair-spread families, then added cached OI-pressure continuation sweeps including a 1,382,400-spec finite grid that promoted G129395/G129183 over the earlier G8020 candidate.
- Mapped competencies: Quant hypothesis expansion, exhaustive parameter search, factor diversification, walk-forward metric comparison, evidence-based candidate selection.
- Next skill to sharpen: Convert the OI-pressure candidate into a Bitget paper emulator and validate live OI/candle alignment before promotion beyond paper.

## 2026-05-07 - G7822 entry-frequency repair analysis
- Conversation/topic: Tested whether G7822's sparse CH1 entry rate can be repaired without losing its quality edge over G1165.
- What was done: Added and ran `g7823_frequency_repair_search.py`, evaluated 19,200 CH1 variants, found no 100+ trade balanced-pass candidate, and documented why the practical solution is G7822 for quality plus G4692 as the high-frequency non-CH1 paper-observation sleeve.
- Mapped competencies: Quant parameter search, trade-frequency/expectancy tradeoff analysis, validation gate design, negative-result interpretation, strategy portfolio construction.
- Next skill to sharpen: Monitor combined paper-fill cadence and attribution for G7822/G4692 so live runtime evidence can confirm the backtest frequency split.

## 2026-05-07 - G7822/G4692 frequency bundle packaging
- Conversation/topic: Converted the G7822 frequency solution from an analysis conclusion into a concrete paper-observation bundle.
- What was done: Added `g7822_g4692_frequency_solution.py`, verified 684 combined trades, 13.33 trades/month, all periods positive, zero liquidations, and zero direct overlap, then packaged the G7822+G4692 bundle card and overrides.
- Mapped competencies: Strategy portfolio construction, validation automation, signal overlap analysis, operational packaging, technical communication.
- Next skill to sharpen: Compare bundle-level expected cadence with real Oracle paper closed-trade timestamps and monitor whether G4692 dominates risk during quiet CH1 periods.

## 2026-05-07 - High-cadence frequency target correction
- Conversation/topic: Reassessed the G7822/G4692 bundle after recognizing that 13.33 entries/month is still too slow for fast paper validation.
- What was done: Corrected the frequency target to 30+ entries/month, distinguished monthly PnL from monthly entry count, selected G264402 from the OI-pressure grid as the highest-cadence retained all-positive zero-liquidation candidate, and packaged a revised G7822/G4692/G264402 bundle with estimated 32.22 entries/month.
- Mapped competencies: Metric hygiene, quant candidate triage, cross-family strategy bundling, evidence correction, operational risk communication.
- Next skill to sharpen: Adapt the G129183 OI-pressure Oracle runtime to G264402 and compare live OI-trigger cadence against the grid estimate.

## 2026-05-07 - Full-window frequency validation correction
- Conversation/topic: Rechecked the proposed high-cadence fix under the same full-window standard as G7822/G4692.
- What was done: Added `g7824_full_frequency_validation.py`, verified OI coverage only spans 2025-03-25 to 2026-04-04, rejected G264402 as a full-window fix, and showed the best full-price add-ons reach only 25.33-26.64 unique entries/month after overlap.
- Mapped competencies: Data coverage auditing, validation standard enforcement, overlap-adjusted metric design, negative-result documentation, technical risk correction.
- Next skill to sharpen: Design a non-breakout full-window sleeve that adds at least 3.36 unique monthly entries without weakening period consistency.

## 2026-05-07 - Multi-sleeve frequency repair beam search
- Conversation/topic: Solved the G7822 paper-frequency target by expanding from single add-on testing to overlap-adjusted multi-sleeve bundle search.
- What was done: Added `g7825_frequency_bundle_beam_search.py`, evaluated pre-gated full-window G1307/G1309/G1304 candidates, used timestamp/symbol/side de-duplication, and found the G7822+G4692+G4475+G4673+G4472 bundle at 31.80 unique entries/month with zero tested liquidations.
- Mapped competencies: Search-space pruning, combinatorial optimization, overlap-aware metric design, validation automation, risk communication.
- Next skill to sharpen: Convert the bundle into a paper runtime with duplicate-entry locks and family exposure caps before any live-facing decision.

## 2026-05-07 - Strict quality-frequency feasibility check
- Conversation/topic: Tested whether the frequency solution can also improve G1165/G7822-level win rate and drawdown.
- What was done: Added `g7826_quality_frequency_strict_search.py`, searched multi-sleeve full-window combinations with strict WR/DD/PnL/frequency gates, found zero all-metric passers, and isolated the blocker: high-cadence sleeves top out around 58-59% WR.
- Mapped competencies: Constraint feasibility testing, metric tradeoff diagnosis, quantitative risk communication, experiment boundary setting.
- Next skill to sharpen: Develop a genuinely new high-frequency/high-WR alpha family or regime filter rather than continuing to stack breakout-family cadence sleeves.

## 2026-05-07 - High-WR repair attempts
- Conversation/topic: Tried additional methods to repair win rate while preserving the solved frequency target.
- What was done: Added G7827/G7828/G7829/G7830 scripts to test new trend/reclaim alpha families, symbol/hour filters, high-WR pocket bundles, and TP/SL exit tuning; found one filtered 65.13% WR sleeve at 8.05 trades/month but no full-window method that reaches G1165/G7822 WR while preserving high cadence.
- Mapped competencies: Iterative experiment design, feature/filter search, exit-profile tuning, constraint diagnosis, evidence-backed rejection.
- Next skill to sharpen: Bring in genuinely different full-window data sources such as OI, liquidations, order book imbalance, or 5m cross-asset lead/lag rather than continuing with 1h price breakout variants.

## 2026-05-07 - Expanded data alpha search status
- Conversation/topic: Challenged whether the prior "not found" conclusion really used all available methods and data.
- What was done: Audited local 5m/funding/OI/liquidation coverage, ran existing 5m CH1 validation, added wide 5m lead-lag/funding/G090 variant scripts, and found a promising G090 hour-filter sleeve with 8.08 trades/month and 95.56% WR across 2024-2026.
- Mapped competencies: Data coverage audit, alternative alpha-source exploration, resource-aware experiment design, honest limitation reporting.
- Next skill to sharpen: Convert wide searches into chunked resumable jobs so timeout-limited interactive runs do not truncate the search surface.

## 2026-05-08 - Oracle paper strategy mid-check
- Conversation/topic: Checked the current Oracle-hosted paper strategy fleet and separated runtime health from strategy performance.
- What was done: Verified SSH/systemd access, active services, state freshness, open/closed trades, PnL, fetch errors, and VM resource headroom for G4692, G7835, G920_top7, and G921_alts.
- Mapped competencies: Production monitoring, paper-trading telemetry interpretation, runtime health diagnostics, evidence-backed strategy checkpointing.
- Next skill to sharpen: Add a single portfolio status script that includes all active strategy service naming variants and unrealized PnL estimates.
