...
def find_targets():
    ...
    for i, b in enumerate(all_blocks):
        if b['type'] == 'heading_2':
            ...
            if "1260" in text or "2178" in text:
                print(f"FOUND H2: {text} at index {i}, ID: {b['id']}")
                # Look for the code block after it
                for j in range(i+1, min(i+5, len(all_blocks))):
                    if all_blocks[j]['type'] == 'code':
                        print(f"  -> CODE BLOCK at index {j}, ID: {all_blocks[j]['id']}")
                        break
